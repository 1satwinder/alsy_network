    ...
    <table
        ...
        <tr>
            ...
            <td colspan="5" style="padding: .85rem 0; vertical-align: top;white-space:normal;line-height: 20px">
                ...
                {{ optional($invoice->state)->name }}-{{ $invoice->pincode }}<br>
                M: {{ $invoice->phone }} <br>
                E: {{ $invoice->email }} <br>
            </td>
        </tr>
        ...
        @foreach($invoice->products()->with('product.GSTType')->get() as $key => $invoiceProduct)
            <tr>
                ...
                <td style="vertical-align: middle;border: 1px solid #ced0d2;padding: .25rem;font-size: 12px;text-align: center">{{ $invoiceProduct->product->GSTType->hsn_code }}</td>
