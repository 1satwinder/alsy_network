<div class="person">
    @if($member)
        <div tabindex="0" data-bs-html="true"
            ...
            @else
                <a href="{{ route('admin.genealogy.show', $member) }}">
                    <img src="{{ $member->present()->genealogyImage() }}"
                         alt="{{ $member->code }}"
                         style="background-color: {{ $member->present()->genealogyImageBackground() }};">
                </a>
        ...
        <div class="name">
            @if(Agent::isMobile())
                <span>
                    <a href="{{ route('admin.genealogy.show', $member) }}">{{ $member->user->name }}</a>
                </span>
                <span>@include('copy-text', ['text' => $member->code])</span>
            @else
                <span class="d-block">{{ ucwords($member->user->name) }}</span>
                <span class="text-nowrap">@include('copy-text', ['text' => $member->code])</span>
            @endif
        </div>
    @else
        <a href="{{ $parent ? route('member.register.create', ['code' => $parent->code, 'side' => $side]) : 'javascript::void(0);' }}" {{ $parent ? 'target="_blank"' : '' }} >
