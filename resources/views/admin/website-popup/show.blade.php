    ...
    <div class="row">
        <div class="col-lg-4">
            <form method="post" onsubmit="subButton.disabled = true; return true;"
                  action="{{ route('admin.website-pop.store') }}" class="filePondForm">
                @csrf
                <div class="card">
                    <div class="card-body">
                        <div class="PopupType">
                            <div class="form-group">
                                ...
                                <div class="form-check form-check-inline mt-1">
                                    <input class="form-check-input popup_type" type="radio"
                                           name="popup_type"
                                           id="self"
                                           value="{{\App\Models\WebsitePopup::TYPE_IMAGE}}"
                                           {{ old('popup_type',request()->get('popup_type')) == \App\Models\WebsitePopup::TYPE_IMAGE ? 'checked' : '' }}
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input popup_type" type="radio"
                                           name="popup_type"
                                           id="other"
                                           value="{{\App\Models\WebsitePopup::TYPE_VIDEO}}"
                                           {{ old('popup_type',request()->get('popup_type')) == \App\Models\WebsitePopup::TYPE_VIDEO ? 'checked' : '' }}
                ...
                <form method="post" onsubmit="subButton{{$index}}.disabled = true; return true;"
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check form-check-inline mt-1">
                                    <input class="form-check-input popup_type{{$index + 1}}" type="radio"
                                           ...
                                           id="image"
                                           value="{{\App\Models\WebsitePopup::TYPE_IMAGE}}"
                                           {{ old('popup_type') == \App\Models\WebsitePopup::TYPE_IMAGE ? 'checked' : '' }}
                                           {{ request()->get('popup_type') ? 'disabled' : '' }}
                                           {{$popup->getFirstMediaUrl(\App\Models\WebsitePopup::MEDIA_COLLECTION_IMAGE_WEBSITE_POPUP) ? 'checked' : ''}}
                                           disabled>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input popup_type{{$index + 1}}" type="radio"
                                           ...
                                           id="video"
                                           value="{{\App\Models\WebsitePopup::TYPE_VIDEO}}"
                                           {{ old('popup_type',$popup->link) == \App\Models\WebsitePopup::TYPE_VIDEO ? 'checked' : '' }}
                                           {{$popup->link ? 'checked' : ''}}
                                           disabled>
                            ...
                            <div class="form-group mb-3">
                                <input type="file" class="filePondInput" name="image" accept="image/*"
                                       data-url="{{ $popup->getFirstMediaUrl(\App\Models\WebsitePopup::MEDIA_COLLECTION_IMAGE_WEBSITE_POPUP) }}"/>
                                @foreach($errors->get("image") as $error)
                                    <span class="text-danger">{{ $error }}</span>
                                @endforeach
                            </div>
                            ...
                            <div class="form-group mb-3 d-flex justify-content-around">
                                <div class="radio radio-info form-check-inline ">
                                    <input type="radio" id="active{{ $index }}"
                                           value="{{ \App\Models\WebsitePopup::STATUS_ACTIVE }}"
                                           name="status"
                                        {{ $popup->status == \App\Models\WebsitePopup::STATUS_ACTIVE ? 'checked="checked"' : '' }}
                                    >
                                    <label for="active{{ $index }}"> Active </label>
                                </div>
                                <div class="radio form-check-inline">
                                    <input type="radio" id="inActive{{ $index }}"
                                           value="{{ \App\Models\WebsitePopup::STATUS_INACTIVE }}"
                                           name="status"
                                        {{ $popup->status == \App\Models\WebsitePopup::STATUS_INACTIVE ? 'checked="checked"' : '' }}
                                    >
